...
14. Dashboard renamed **Nubra Beacon** (user-picked, 2026-07-05) with official nubra.io logo mark; light theme added (System/Dark/Light, persisted, no-flash boot script); Next dev indicator disabled. **Beacon requests** page (`/requests`, sidebar group "Improve"): team logs asks for new dashboard sections/capabilities into the designed `feedback` table (`GET`/`POST /api/v1/feedback`, category `ui_feature_request`) — deliberately NOT on the community Features page, which stays measured signal only.
